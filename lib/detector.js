...
function greetingReply() {
  return 'Hi 👋 I’m Atom. Tell me what you need and I’ll keep it clear.';
}
...
function hasMedicalSignal(text = '', data) {
  ...
  const medicalTerms = [
    ...
    'emergency', 'urgent', 'first aid', 'trauma', 'bleeding', 'fainted', 'unconscious', 'seizure', 'breathing difficulty', 'shortness of breath', 'chest pain',
    'drug', 'drugs', 'medicine', 'medicines', 'medication', 'medications', 'pill', 'tablet', 'capsule', 'dose', 'dosage', 'side effect', 'adverse', 'interaction', 'contraindication', 'contraindicated', 'pharmacy', 'pharmacology', 'pharmacist', 'clinical', 'patient', 'case', 'lab', 'labs', 'diagnosis', 'symptom', 'symptoms', 'treatment', 'therapy', 'monitoring', 'guideline', 'otc', 'rx', 'prescription', 'safe', 'safety', 'toxicity', 'toxic', 'clinical trial',
    // Case follow-up / emergency terminology that users often paste from an answer
    'aki', 'acute kidney', 'ckd', 'egfr', 'scr', 'creatinine', 'serum creatinine', 'renal', 'renal function', 'kidney', 'nephrotoxin', 'nephrotoxins', 'nephrology', 'urine output', 'oliguria', 'fluid management', 'cautious fluid',
    'potassium', 'hyperkalemia', 'hypokalemia', 'sodium', 'hyponatremia', 'electrolyte', 'calcium gluconate', 'insulin/glucose',
    'qtc', 'qt prolongation', 'torsades', 'tdp', 'telemetry', 'bradycardia', 'tachycardia', 'hypotension', 'hypertension', 'heart failure', 'hfrEF'.toLowerCase(), 'arrhythmia', 'palpitations',
    'gi bleed', 'gastrointestinal bleeding', 'melena', 'dark stools', 'black stool', 'blood in stool', 'hb', 'hemoglobin', 'type and cross', 'endoscopy', 'ppi', 'anticoagulation', 'anti-xa', 'inr',
    'lactate', 'metformin-associated lactic acidosis', 'sepsis', 'pneumonia', 'crp', 'wbc', 'alt', 'ast', 'liver', 'hepatic',
    'prescriber', 'clinician', 'cardiology', 'gastroenterology', 'icu', 'ed transfer', 'escalation', 'hold', 'stop', 'avoid', 'restart', 'alternative',
    'active ingredient', 'excipient', 'formulation', 'manufacturing', 'herb', 'plant', 'mechanism of action', 'class of',
    // Arabic / Egyptian Arabic
    'دواء', 'ادوية', 'أدوية', 'دوا', 'علاج', 'أقراص', 'اقراص', 'كبسول', 'حقن', 'شراب', 'جرعة', 'اعراض', 'أعراض', 'عرض جانبي', 'اثار جانبية', 'آثار جانبية', 'تفاعل', 'تداخل', 'صيدلة', 'صيدلي', 'مريض', 'حالة', 'الحالة', 'تحاليل', 'تحليل', 'تشخيص', 'مضاد', 'مسكن', 'حامل', 'حمل', 'ضغط', 'سكر', 'كلى', 'كلية', 'كلوي', 'قصور كلوي', 'كرياتينين', 'بوتاسيوم', 'صوديوم', 'كبد', 'حساسية', 'موانع', 'متابعة', 'مذاكرة فارما', 'فارما', 'كلينيكال', 'طبي', 'ميديكال', 'مادة فعالة', 'مادة إضافية', 'مادة اضافية', 'سواغ', 'تصنيع', 'تركيبة', 'طوارئ', 'اسعاف', 'إسعاف', 'نزيف', 'براز اسود', 'براز أسود', 'اغماء', 'إغماء', 'تشنج', 'اختناق', 'حادث', 'وقعة', 'كسر', 'جرح', 'بيتنفس', 'مش بيتنفس', 'ينفع', 'آمن', 'امن', 'خطر', 'ضرر', 'اضرار', 'أضرار', 'فوائد', 'بديل', 'يتاخد', 'يتاخد مع', 'معاه', 'معاها', 'رسم قلب', 'هيموجلوبين', 'سيولة', 'مميع', 'نزيف معدة', 'طوارىء', 'طوارئ'
  ];
  return medicalTerms.some(term => t.includes(String(term).toLowerCase()));
}

function isContinuationRequest(text = '') {
  const t = String(text || '').trim().toLowerCase().replace(/[؟?!.]+$/g, '').trim();
  if (!t) return false;
  return /\b(continue|resume|finish|complete|go on|carry on|where were you|you stopped|stopped at|cut off|truncated)\b/i.test(t)
    || /(كمل|كمله|كمّل|اكمل|أكمل|استكمل|كمل باقي|كمل الباقي|كملينا|فين تكملة|فين تكمله|وقف عند|اتقطع|مكملتش|كملت؟|باقي الكلام|باقى الكلام)/.test(t);
}

function isLikelyFollowUp(text = '') {
  const t = String(text || '').trim().toLowerCase();
  if (!t) return false;
  if (isContinuationRequest(t)) return true;
  if (t.length <= 100 && /^(why|how|and|so|ok|okay|then|what about|what if|explain more|continue|ليه|ازاي|إزاي|طب|طيب|تمام|وضح|اشرح|يعني|وبعدين|والبديل|البديل|ينفع|امتى|مينفعش|معاه|معاها|ده|دي|دول|هو|هي|كده|اها|اه)$/.test(t.replace(/[؟?!.]+$/g, '').trim())) return true;
  return t.length <= 220 && /(ليه|ازاي|إزاي|طب|طيب|يعني|كمل|اكمل|أكمل|وضح|اشرح|ينفع|مينفعش|بديل|معاه|معاها|ده|دي|دول|what about|what if|why|how|continue|resume|finish|stopped|cut off)/.test(t);
}

function isClearlyNonMedical(text = '') {
  const t = String(text || '').toLowerCase();
  if (!t.trim()) return false;
  if (hasMedicalSignal(t, { aliases: {}, monographs: {} }) || isContinuationRequest(t)) return false;
...
function isMedicalInScope(text = '', data, contextText = '') {
  ...
  if (isContinuationRequest(latest) && hasMedicalSignal(combined, data)) return true;
  if (isLikelyFollowUp(latest) && hasMedicalSignal(combined, data)) return true;
  if (isVagueHumanQuestion(latest)) return true;

  // Soft guard: ambiguous messages should reach the model. Hard blocking makes follow-up clinical chat feel broken.
...
module.exports = { isShortGreeting, greetingReply, isVagueHumanQuestion, vagueHumanClarificationReply, isGeneralKnowledgeQuestion, detectModeFromText, hasMedicalSignal, isContinuationRequest, isMedicalInScope, outOfScopeReply };
